...
浮動小数点数を使った数値計算では、数値の変換や演算のたびに丸め誤差が発生する。
特に複数回演算操作を行った後、誤差がどう伝搬していくかは、直感的には分かりにくい問題だ。
この記事では、IEEE 754-2019をもとに浮動小数点数の定義を確認し、誤差推定の基本的な方法について議論していこう。

## 浮動小数点の仕組み

まず、浮動小数点数には大きく２つのタイプ、バイナリ浮動小数点とデシマル浮動小数点が存在する。
